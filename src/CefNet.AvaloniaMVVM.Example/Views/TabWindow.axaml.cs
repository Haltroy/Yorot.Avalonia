...
#pragma warning restore IDE0044 // Add readonly modifier
        ...
        private void RefreshMainWindow()
        {
            Avalonia.Threading.Dispatcher.UIThread.InvokeAsync(() =>
            {
                ...
                {
                    mainWindow.reload.IsVisible = IsNavigated;
                    mainWindow.stop.IsVisible = IsNavigating;
                    mainWindow.goback.IsEnabled = CanGoBack;
                    mainWindow.goforward.IsEnabled = CanGoForward;
                    mainWindow.zoomin.IsEnabled = CanZoomIn;
                    mainWindow.zoomout.IsEnabled = CanZoomOut;
                    mainWindow.favoritedbutton.IsVisible = IsFavorited;
                    mainWindow.favoritebutton.IsVisible = IsNotFavorited;
                    mainWindow.UnmuteButton.IsVisible = IsMuted;
                    mainWindow.MuteButton.IsVisible = IsUnmuted;
                    mainWindow.WebsiteGood.IsVisible = IsPageSafe;
                    mainWindow.PageSafeInfo.IsVisible = IsPageSafe;
                    mainWindow.WebsiteMeh.IsVisible = IsPageUsedCookie;
                    mainWindow.PageUsedCookieInfo.IsVisible = IsPageUsedCookie;
                    mainWindow.WebsiteBad.IsVisible = IsPageUnsafe;
                    mainWindow.PageUnsafeInfo.IsVisible = IsPageUnsafe;
                    mainWindow.ZoomLevel.Text = ZoomLevel;
                    mainWindow.tbUrl.Text = tbUrl;
                    mainWindow.FindCount.Text = FindCount;
        ...
        private void TabWindow_Initialized(object? sender, EventArgs e)
        {
            SessionSystem.LoadPage += (url) =>
            {
                if (webView1 != null) { webView1.Navigate(url); }
            };

            SessionSystem.Add(new(_startUrl));
            if (SessionSystem.Count > 0)
            {
